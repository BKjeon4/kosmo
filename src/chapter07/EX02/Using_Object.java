...
class A {		// 클래스 : 객체를 생성하기 위한 설계도
	//클래스 내부에 올 수 있는 것(3가지) : 1. 필드 (클래스에서 선언된 변수), 2. 생성자, 3.메소드, 4.이클래스
	//1. 필드
	int m;		// heap 공간에 저장, 강제 초기화
	int n;		//0
	double k;	//0.0
	String l;	//null
	//기본 값을 입력하지 않으면 자동으로 기본값으로 생성 됨
	
	//생성자 : 클래스는 반드시 생성자가 존재해야 한다.
	//특징 : 1. 클래스 이름과 동일, 2. 리턴이 존재하지 않아야 된다.
	...
	//메소드	: 리턴타입 메소드명 (입력매개변수) {실행코
	//		프로그램의 기능을 코드로 작성
	//		호출해서 작동됨, work1()
	void work1 () {
		System.out.println("work1 메소드호출");
	}
