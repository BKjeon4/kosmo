...
public class Using_Array {
	...
	public static void main(String[] args) {
		...
		int[] a = new int[5];	// a는 참조변수 ( 참조 주소값을 가지고 있다. )
			// 배열 a 변수의 방의 크기 5개 생성 후 주소값을 a에 할당 해라
			// 방번호 [index] 0 ~ 4 방까지 Heap 영역에 생성 됨 
			// new : Heap 영역의 배열 방 5개를 생성해서 Heap 영역의 번지를 a 변수에 할당해라
		
		
		// 배열 변수의 값을 할당
		a[0] = 10;	// Heap 영역의 index 0 방에 점수 10을 넣어라
		a[1] = 20;
		a[2] = 30;
		a[3] = 40;
		a[4] = 50;
		...
		System.out.println("배열변수 a 방의 갯수 : " + a.length);		// 배열 방의 총 갯수를 출력
		...
		System.out.println(Arrays.toString(a));		// [ , ] (대괄호) 안에 있는 값을 출력
	}
