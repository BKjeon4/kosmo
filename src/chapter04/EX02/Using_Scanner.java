...
public class Using_Scanner {
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	- Scanner sc를 사용 후에는 sc.close()를 사용해서 객체를 메모리에서 제거 해주어야한다.
		 
		 */
		
		Scanner sc = new Scanner (System.in);		// 1. 마우스 오버로 scanner 임포트 2. ctrl + shift + O
			//Scanner 클래스로부터 sc라는 객체를 생성함. (sc : 콘솔을 통해서 인풋받음)
			//Ctrl + shift + O : 외부 클래스 자동 import
		
			//sc는 객체
			//sc.nextint() : 객체의 함수(method)
		System.out.println("반드시 정수를 입력 하세요>>>");
				
		int a = sc.nextInt();		//콘솔에서 정수값을 받아서 변수 a에 할당
		System.out.println("변수 a를 출력 : " + a);
		
		System.out.println("반드시 문자열을 입력 하세요>>>");
